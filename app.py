...
def fetch_from_wikipedia(item_name):
    ...
    try:
        ...
        for result in results:
            ...
            # (handles compound words like "applesauce" matching "apple" + "sauce")
...
def fetch_product_image(item_name, description='', store=''):
    """Search for a product image: store-specific → Open Food Facts → Google (if configured)."""
    ...
    # Try Google Custom Search API (uses GOOGLE_API_KEY and GOOGLE_CX env vars if set)
...
def refresh_image(item_id):
    ...
    # Clear cached result so refresh actually re-searches
...
def upload_item_image(item_id):
    ...
    if ext not in ('.jpg', '.jpeg', '.png', '.webp'):
        ext = '.jpg'
    # Use a stable filename so the same item always maps to the same file
    filename = f"custom_{item_id}{ext}"
    local_path = os.path.join(app.config['IMAGE_CACHE'], filename)
    file.save(local_path)
    # Write to image_cache table keyed by item name so future receipt scans
    # of the same item automatically pick up this custom image
    query = f"{item['name']} {item['description'] or ''}".strip()
    query_hash = hashlib.md5(query.lower().encode()).hexdigest()
    conn.execute(
        'INSERT OR REPLACE INTO image_cache (query_hash, query, local_path, source_url, date_cached) VALUES (?,?,?,?,?)',
